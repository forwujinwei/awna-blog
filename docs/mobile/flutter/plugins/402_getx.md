...
## get_cli使用
### 安装Cli脚手架
我们通过命令`flutter pub global activate get_cli` 进行脚手架的全局安装，本文以Mac OS为例。
...
![image](https://gitee.com/Awna/pic/raw/master/webp)

### 代码结构分析
![image](https://gitee.com/Awna/pic/raw/master/image-20211006183739992.png)

这个结构类似于mvc的思想，binding相当于m,controller就是c,view就是v

初始化出的项目data是空的，后续会使用到

### Getx_pattern和CLEAN说明
#### 第一种：Getx_pattern
...
```
get create page:news
```
![image](https://gitee.com/Awna/pic/raw/master/image-20211006184910285.png)

### Cli创建控制器
我们可以通过`get create controller:login`来快速为已存在的page进行创建`controller`，并且他还重新了controller的生命周期以及初始化方法
...
#### 添加国际化文件
在assets/locales文件夹下创建如下文件，并添加文件内容
> zh_CN.json
```
{
    "buttons": {
...
JMdeMacBook-Pro:getx_example jm$ 
```
![image](https://gitee.com/Awna/pic/raw/master/image-20211006190013762.png)

### Cli通过json生成模型
#### 添加文件
在assets下创建文件夹models，在models目录下创建news.json,并添加文件
```
{
    "stat": "1",
    "data": [
